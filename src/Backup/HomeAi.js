...
const Home = props => {
  ...
  const fetchLeaderboardData = async userId => {
    ...
    try {
      ...
      if (result.status === '200') {
        const pointToDisplay = result.user_point.toString();
  
        dispatch({
          type: ACTIONS.SET_RANKING_DATA,
          payload: {
            rank: result.user_rank || 'N/A',
        point: pointToDisplay, // This should be the correct points
        profileImage: `${result.user_img}${userId}.png`,
          },
        });
        console.log('Dispatched payload:', {
          rank: result.user_rank || 'N/A',
          point: pointToDisplay,
          profileImage: `${result.user_img}${userId}.png`,
        });
      } else {
        dispatch({
          type: ACTIONS.SET_ERROR,
          payload: result.message || 'No ranking data available',
        });
      }
    } catch (error) {
      ...
      dispatch({
        type: ACTIONS.SET_ERROR,
        payload: 'Error fetching leaderboard: ' + error.message,
      });
    } finally {
  ...
  const loadUserIdAndFetchLeaderboard = async () => {
    try {
      const storedUserId = await AsyncStorage.getItem('userId');
      if (storedUserId) {
        dispatch({type: ACTIONS.SET_USER_ID, payload: storedUserId});
        await fetchLeaderboardData(storedUserId);
      } else {
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={'height'}>
          <View
            ...
            <Text
              ...
              {`point: ${state.userPoints !== null ? state.userPoints : '800'}`}
            </Text>
            </View>
          </View>
