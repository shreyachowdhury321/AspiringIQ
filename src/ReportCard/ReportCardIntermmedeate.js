import {
  ...
  const ReportCardIntermedeate = (props) => {
    ...
    const userId = props.route.params?.userId || '43'; // Pass userId dynamically
    const attempid = props.route.params?.attempid || '1'; // Pass attempid dynamically
    const level = props.route.params?.level || 'Intermedeate'; // Pass level dynamically
    ...
    useEffect(() => {
      ...
      fetchData();
    }, [userId, attempid, level]); // Fetch data when userId, attempid, or level changes
  ...
  export default ReportCardIntermedeate;
